...
# 80/20 train/inference split
TRAIN_INFERENCE_RATIO = 0.8
...
def split_train_inference(
    n_samples: int,
    train_ratio: float = TRAIN_INFERENCE_RATIO,
    seed: int = SPLIT_SEED,
) -> tuple[list[int], list[int]]:
    """Shuffled 2-way train/inference split (NO val) for the PROPOSED
    methods (bandit / submodular / two_stage) -- these never expose a
    held-out validation split to any acquisition/budget process, so
    there's nothing to carve one out for. Same random.Random(seed).shuffle
    mechanics as split_dataset (so a given seed is still a single
    reproducible partition), just a straight train_ratio/[1-train_ratio]
    cut instead of a 3-way split.

    NOTE: because the cut point differs from split_dataset's (80% vs 60%
    train), the resulting train/inference index sets are NOT the same as
    split_dataset(seed)'s train/test sets at the same seed, even though
    the underlying shuffle order is identical -- this trades exact
    train/test alignment with the CAE/EDDI/DIME baselines for a simpler,
    val-free split that better matches what the proposed methods actually
    need.
    """
    ...
    random.Random(seed).shuffle(all_indices)
    train_indices = all_indices[:train_size]
    inference_indices = all_indices[train_size:]
    return train_indices, inference_indices
...
# (gmm_bandit.gmm_bandit_runner, gmm_submodular.gmm_submodular_runner,
...
# in gmm_bandit_runner.py and gmm_submodular_runner.py (and two_stage_runner.py
