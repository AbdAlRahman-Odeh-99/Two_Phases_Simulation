...
synthetic GMM datasets including synthetic) --
    ...
    --method adaptive  -> adaptive.adaptive_runner.run_experiment
    --method two_stage -> two_stage.two_stage_multiclass_runner.run_experiment
...
=== Observability ===
Every run has an identity (run_id) shared by its enabled artefacts:

    logs/{run_id}.log             optional (--file-log), same configured level
    results/{run_id}.manifest.json  optional (--json-sidecars): git commit +
                                    dirty flag, argv, resolved
                                    args, package versions, hostname, PBS
                                    job id, timing, failures
    results/{run_id}.rows.jsonl     optional (--json-sidecars): every row,
                                    appended and flushed as it is produced
...
derived from the output workbook's stem, so enabled artefacts sort together.
"""
...
import two_stage.two_stage_multiclass_runner as two_stage_mc_greedy_runner
...
the same "greedy" / "lp_chain+subsets" / "lp_full+selected" strings
    ...
    for d in all_results:
        row = {
            ...
            "Dataset": dataset_name,
            "Split Mode": d.get("split_mode", np.nan),
            "Seed": _nan(d, "seed"),
...
UNIFIED_COLUMNS = [
    "Method", "Feedback", "Num Classes", "Warm Start", "Dataset", "Split Mode", "Seed",
    ...
    "Num Masks Inference",
    "Acquisition", "Reward Estimate", "Avg Views Acquired", "Num Arms",
...
def run_inference_from_saved_states(source):
    ...
    if method == "two_stage":
        native_rows = two_stage_mc_greedy_runner.run_inference_from_states(source)
        return normalize_two_stage(
            native_rows, dataset, "two_stage",
            acquisition=_acquisition_label(
                config.get("acquisition", "greedy"),
                config.get("reward_update", "subsets"),
                config.get("reward_estimate", "surrogate")),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-xlsx", type=str, default=None,
                         help="If not given, auto-named as results/results_{method}_"
                              "{dataset}_V{N or ALL}_T{n_seeds}.xlsx ('ALL' when "
