...
from core.utils import restore_parameters, generate_uniform_mask
...
class IterativeSelector(nn.Module):
    ...
    def fit(self,
        ...
        Args:
          ...
          verbose:
          feature_costs: per-feature cost tensor/array, feature_costs[0]
            == 0 (free feature). REQUIRED if budget_state is given.
          budget_state: optional budget_state.BudgetState. If given
            (together with feature_costs), TRAINING masks are drawn via
            MaskPool -- epoch 0 spends budget_state's total_budget
            EXACTLY ONCE (single pass, budget-constrained masks), then
            every later epoch just replays a fresh permutation of that
            same fixed pool with no further spending -- same mechanism
            as MaskingPretrainerPL2/PVAE.fit. VALIDATION masks are left
            unchanged (still uniform), matching those files' convention
            of not budget-biasing the early-stopping/model-selection
            signal. If None (default), behavior is UNCHANGED.
        '''
        ...
        if budget_state is not None:
            # Same fix as pvae.py's fit() / MaskingPretrainerPL2 -- this
            # used to call budget_state.reset_pool() every epoch (a real
            # bug: gives EVERY epoch a fresh full budget, so cumulative
            # spend grows roughly linearly with epoch count instead of
            # being capped once for the whole run). Now: epoch 0 spends
            # budget_state's total_budget EXACTLY ONCE (single pass over
            # all samples, budget-constrained mask per batch); the
            # resulting masks are cached into a fixed pool via MaskPool.
            # From epoch 1 onward, NO further spending happens -- each
            # epoch just gets a fresh random PERMUTATION of that same
            # pool paired against whatever batches train_loader yields,
            # so the model still sees varied sample<->mask pairings every
            # epoch (mask diversity) without ever exceeding the budget
            # spent once in epoch 0. See MaskPool's docstring in
            # budget_state.py.
