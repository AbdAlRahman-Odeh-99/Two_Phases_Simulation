...
def get_linear_network(d_in, d_out):
    '''
    Linear classifier: a single affine map, no hidden layers, no
    nonlinearity -- logistic/softmax regression when trained with
    CrossEntropyLoss (which applies softmax internally), matching how
    get_mlp_network's output is consumed everywhere else in this
    codebase. Same call signature as get_mlp_network (minus hidden/
    dropout, which don't apply to a linear model) so it's a drop-in
    swap at any predictor call site.
    '''
