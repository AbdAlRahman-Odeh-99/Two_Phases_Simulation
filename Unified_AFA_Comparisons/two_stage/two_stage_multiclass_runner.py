...
timestamp and a level and go to the console. Passing --file-log also writes
logs/{run_id}.log; the shared --log-level threshold controls both.
...
def run_experiment(
    ...
    run = get_run()
    state_dir = state_directory(run=run, state_dir=state_dir, method="two_stage")
    ...
    for seed in seeds:
        ...
        log.info("Split %s: train=%d, validation=%d, test=%d",
                 split_mode, n_train, len(val_idx), n_test)
        ...
        for budget_fraction in budget_fractions:
            ...
            for init_fraction in init_fraction_grid(budget_fraction, n_init_fraction_points):
                ...
                with cell_timers(inherit=("t_data_load",)) as timers, \
                    ...
                    # Save after every completed training cell at the exact
                    # RNG boundary immediately preceding inference.
                    training_row = {
                        'seed': seed,
                        'warm_start': False,
                        'nclasses': nclasses,
                        ...
                        'Selected Subsets': (
                            "" if trace_rounds else serialize_selected_subsets(
                        ...
                        'n_budget_fallbacks_train': stage2_result.get(
                            'n_budget_fallbacks', np.nan),
                        'pred_rule': pred_rule,
                    }
                    state_path = save_training_state(
                        method="two_stage", cell=cell, run=run,
                        state_dir=state_dir, rng=rng,
                        arrays={
                            "learned_centers": inference_centers,
                            "costs": costs,
                            "X_inference": X_test,
                            "Y_inference": Y_test,
...
def run_inference_from_states(source):
    """Run two-stage inference from saved cells without rerunning training."""
    run = get_run()
    paths = find_training_states(source, method="two_stage")
    if not paths:
        raise ValueError(f"no two_stage training states found in {source}")
    rows = []
    progress = Progress(len(paths), label="states", logger=log)
    for path in paths:
        meta, arrays = load_training_state(path, expected_method="two_stage")
        cell = dict(meta["cell"])
        row = None
        with cell_timers() as timers, guard(cell, logger=log) as outcome:
            rng = restored_rng(meta)
            training_row = dict(meta["training_row"])
            inference_start = time.time()
            inference_result = run_inference_lp_dataset_colgen_multiclass(
                X_inference=arrays["X_inference"],
                Y_inference=arrays["Y_inference"],
                learned_centers=arrays["learned_centers"],
                costs=arrays["costs"],
                inference_budget=float(training_row["inference_budget"]),
                rng=rng,
                pred_rule=training_row.get("pred_rule", "nearest_center"),
            )
            n_init = int(training_row["T1"])
            n_train = int(training_row["n_train"])
            n_test = int(training_row["n_test"])
            total_reward = (
                (1 - training_row["init_error"]) * n_init
                + training_row["avg_reward"] * training_row["T2"]
                + inference_result["inference_accuracy"] * n_test
            ) / max(1, n_train + n_test)
            row = training_row
            row.update({
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split-mode", choices=SPLIT_MODES, default="80-20",
                        help="80-20 for comparing two-stage with adaptive; "
